...
// src/infraestructure/http/people/get.ts
var get_exports = {};
...
// src/adapters/repository/models/people.repository.ts
var PeopleRepository = class {
  ...
  savePeople = async (person) => {
    ...
    try {
      ...
      console.log("Persona creada con \xE9xito:", person);
      return person;
    } catch (error) {
...
// src/adapters/swapi/swapi.adapter.ts
var BASE_URL = "https://swapi.py4e.com/api";
var peopleMapper = (person) => ({
  nombre: person.name,
...
var getPeopleSwapi = async (id) => {
  try {
    const response = await axios_default.get(`${BASE_URL}/people/${id}`);
...
// src/domain/entities/people.entity.ts
var Person = class {
  constructor(id, nombre, altura, peso, colorDePelo, colorDePiel, colorDeOjos, fechaDeNacimiento, genero) {
    this.id = id;
    this.nombre = nombre;
    this.altura = altura;
    this.peso = peso;
...
// src/domain/usecases/people/get-people.usecase.ts
var GetPeople = class {
  constructor(peopleRepository2) {
    this.peopleRepository = peopleRepository2;
  }
  execute = async (id) => {
...
var createPersonUseCase = new GetPeople(peopleRepository);
var handler = async (_event, _context) => {
  var _a;
  console.log("Params: ", _event);
  console.log("Params: ", _event.pathParameters);
  const id = (_a = _event.pathParameters) == null ? void 0 : _a.id;
  if (!id) {
    return {
      statusCode: 403,
